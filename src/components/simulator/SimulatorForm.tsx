...
export function SimulatorForm({ state, etfs, onChange }: SimulatorFormProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-5">
      ...
      {/* Capitale iniziale */}
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Investimento iniziale (€)</label>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Contributo periodico (€)</label>
        <div className="flex gap-2">
          ...
          <select value={state.contributionFrequency}
            ...
            className="w-32 border border-gray-200 rounded-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
      ...
      <div>
        ...
        <div className="flex justify-between text-xs text-gray-400 dark:text-gray-500 mt-0.5"><span>1 anno</span><span>40 anni</span></div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Portafoglio ETF</label>
          {hasPortfolio && (
                ...
                <div key={entry.isin} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button onClick={() => removeEtf(entry.isin)}
                      className="text-xs text-red-400 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 flex-shrink-0">×</button>
                  </div>
            ...
            <div className="flex gap-2 mt-2">
              {Math.abs(totalWeight - 100) >= 0.5 && (
                <button onClick={normalize}
                  className="flex-1 text-xs px-2 py-1 rounded border border-blue-300 dark:border-blue-600 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20">
                  Normalizza a 100%
                </button>
              )}
              <button onClick={clearPortfolio}
                className="text-xs px-2 py-1 rounded border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700">
                Svuota
              </button>
            </div>
          </div>
        ) : (
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">Nessun ETF selezionato: viene usato il rendimento personalizzato qui sotto.</p>
        )}
        ...
        {!hasPortfolio && (
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600 dark:text-gray-400">% annuo (personalizzato)</span>
          </div>
        )}
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Frequenza capitalizzazione</label>
      ...
      {/* Regime fiscale */}
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Regime fiscale</label>
        <div className="flex gap-2">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">L'impatto fiscale lordo è identico: cambia solo il momento del prelievo.</p>
      </div>

      {/* Rebalancing */}
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ribilanciamento</label>
        <div className="flex gap-2">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Imposta di bollo</p>
          <p className="text-xs text-gray-400 dark:text-gray-500">0,20% annuo sul valore del portafoglio</p>
        </div>
      ...
      <div>
        ...
        <input type="range" min={0} max={6} step={0.5} value={state.inflationRate}
          ...
          className="w-full accent-blue-600" />
        <div className="flex justify-between text-xs text-gray-400 dark:text-gray-500 mt-0.5"><span>0%</span><span>6%</span></div>
      </div>
    </div>
  );
}
